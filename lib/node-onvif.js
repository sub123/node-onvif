...
var found_device = []; //scan result

function ping(cur_addr){
	const OnvifServiceDevice = require('./modules/service-device.js');
	let onvfd = new OnvifServiceDevice({ 'xaddr': cur_addr, 'user': '', 'pass': '' })
	let promise = new Promise((resolve, reject) => {
		onvfd._getSystemDateAndTime().then(() => {
			console.log("got reply from " + cur_addr);
			resolve(cur_addr);
		}).catch((error) => {
			reject(error);
		});
	...
	promise.then((cur_addr) => {
		//callback(null, cur_addr);
		found_device.push(cur_addr);
	}).catch((error) => {
		//callback(error);
...
Onvif.prototype.startScan = function(callback){
	let promise = new Promise((resolve, reject) => {
		var os = require('os');
		...
		for(var i in interfaces){
			for(var j in interfaces[i]){
				...
				if(addr.family == 'IPv4'&& !addr.internal){
					console.log(JSON.stringify(addr.address+'/'+addr.netmask));
					addrs.push(addr.address+'/'+addr.netmask);
				}
			}
		}
		console.log("current device addrs =  " + JSON.stringify(addrs));
		console.log("scan_start:");

		ping("http://172.17.0.2:10000/onvif/device_service");
		/*const Netmask = require('netmask').Netmask;
		addrs.forEach(function(addr){
			let block = new Netmask(addr);
			let ports = ["80","443","8880", "10000"];
			block.forEach( (ip, long, index) => {
				//TODO: call get system time or preferably getcapablities
				ports.forEach(function(port){
					let cur_addr = ip + ":" + port;
					ping(cur_addr);
				});
			})
			console.log("found devices at : " + JSON.stringify(found_device));
		});*/
	});
